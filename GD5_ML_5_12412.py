...
st.markdown("""
    ...
    <p style='text-align:center; color:gray; font-size:17px;'>
        Input manual fitur в†’ Prediksi harga rumah (USD & IDR)
    </p>
    <br>
 ...
 # ================================
 st.header("рџ“ќ Input Fitur Rumah")
 ...
 st.markdown(f"""
 **Interpretasi Berdasarkan Input:**- **Overall Quality = {overall_qual}**  
  Level ini {"tinggi" if overall_qual >= 7 else "sedang" if overall_qual >= 5 else "rendah"} dan sangat memengaruhi harga.- **Overall Condition = {overall_cond}**  
  Menjelaskan kondisi struktural & pemeliharaan rumah.- **Gr Liv Area = {gr_liv_area:,} sqft**  
  Rumah seluas ini termasuk kategori {"besar" if gr_liv_area > 2000 else "standar"}.- **Central Air = {central_air}**  
  Kehadiran AC sentral meningkatkan nilai rumah.- **Total Basement SF = {total_bsmt_sf:,} sqft**  
  Basement luas menambah area fungsional.--
###  Akurasi Model (dibaca otomatis dari model)- **MAE Train:** В± **${MODEL_MAE_TRAIN:,.0f}**  - **MAE Test:** В± **${MODEL_MAE_TEST:,.0f}**  - **RВІ Train:** **{MODEL_R2_TRAIN:.2f}**  - **RВІ Test:** **{MODEL_R2_TEST:.2f}**
 **Apa artinya?**- Prediksi model biasanya meleset sekitar **${MODEL_MAE_TEST:,.0f}** dari harga asli.  
